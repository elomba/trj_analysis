module mod_input
   ...
   logical, dimension(5) :: rdf_sq_cl_dyn_conf
   real(myprec) :: deltar, rcl, dcl, qmin, qmax, sigma, pwall, pwallp, rcrdf
   ...
   namelist /INPUT/ log_output_file, trj_input_file, ndim, nsp, nthread, &
      ncfs_from_to, rdf_sq_cl_dyn_conf
   ...
   namelist /INPUT_DYN/ nbuffer
contains

   subroutine read_input_file()
      integer :: io_input_file

      rdf_sq_cl_dyn_conf(:) = .false.
      ...
      deltar = -1.0
      if (rdf_sq_cl_dyn_conf(1) == .true. .or. rdf_sq_cl_dyn_conf(3) == .true.) read (unit=io_input_file, nml=INPUT_RDF)
      qmax = -1.0
      if (rdf_sq_cl_dyn_conf(2) == .true. .or. rdf_sq_cl_dyn_conf(3) == .true.) read (unit=io_input_file, nml=INPUT_SQ)
      rcl = -1.0
      if (rdf_sq_cl_dyn_conf(3) == .true.) read (unit=io_input_file, nml=INPUT_CL)
      if (rdf_sq_cl_dyn_conf(4) == .true.) read (unit=io_input_file, nml=INPUT_DYN)
      idir = 0
      if (rdf_sq_cl_dyn_conf(5) == .true.) read (unit=io_input_file, nml=INPUT_CONF)
