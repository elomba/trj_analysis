...
contains
   ...
   subroutine select_species(nsp,ntypes,Nsites,natoms)
      implicit none
      ...
      if (nsp > ntypes) then
         ...
         write(*,'(/" ** Atom type selected: ",15i3)')sp_types_selected(1:nsp)
...
!         wtypes(1:nsp) = sp_types_selected(1:nsp)
         ...
         if (rigid) then
            ...
            write(*,'("**          All sites should be selected. Temperature incorrectly estimated. !")')
            rigid = .false.
         endif
         run_thermo = .false.
         ex_stress = .false.
   ...
   subroutine basic_init(use_cell,run_clusters,run_dyn,confined,Nsites)
      implicit none
   ...
   subroutine init_modules(use_cell,run_rdf,run_sq,run_clusters,run_order,nsp,Nsites,nbcuda)
      implicit none
      ...
      if (run_order) call order_init(norder,Nsites)
   ...
   subroutine print_results(run_sq,  run_rdf, run_dyn, run_clusters, run_thermo, ntype, nsp, lsmax, Nsites, nqmin, rcl)
      use mod_precision
      ...
      if (ex_qc) write(*,'(/3x,"···   Net charge:",f12.5," e")')sum(qcharge(1:Nsites))
   ...
   subroutine write_spine_lammpstrj(filename, timestep, pos, spine, spine_len, box)
    implicit none
    character(len=*), intent(in) :: filename
    integer, intent(in) :: timestep             ! Current simulation frame/timestep
    real(myprec), intent(in) :: pos(3, *)       ! Global coordinates [x, y, z]
    integer, intent(in) :: spine_len            ! Number of atoms in the spine
    integer, intent(in) :: spine(spine_len)     ! Array of spine atom indices
    real(myprec), intent(in) :: box(3)          ! Box side lengths [Lx, Ly, Lz]
    ...
    ! Use standard F2008 newunit to automatically assign a free file unit
    if (file_exists) then
    ...
    ! 1. Write standard LAMMPS Trajectory Header
    write(iunit, '(A)') 'ITEM: TIMESTEP'
    write(iunit, '(I0)') timestep
    
    write(iunit, '(A)') 'ITEM: NUMBER OF ATOMS'
    write(iunit, '(I0)') spine_len
    
    ! Note: 'pp pp pp' denotes periodic boundaries. 
    ! Adjust the 0.0 bounds if your simulation box is centered at 0 (e.g., -L/2 to L/2)
    ...
    ! 2. Write Atom Data
    ! We output: sequential_id, dummy_type(1), x, y, z
    write(iunit, '(A)') 'ITEM: ATOMS id type x y z'
    
    do i = 1, spine_len
       atom_idx = spine(i)
       ! I8 for ID, I4 for Type, 3F15.6 for coordinates
       write(iunit, '(I8, I4, 3F15.6)') i, 1, pos(1, atom_idx), pos(2, atom_idx), pos(3, atom_idx)
    end do

    close(iunit)
  end subroutine write_spine_lammpstrj
